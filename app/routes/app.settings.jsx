...
import {
  ...
  InlineStack, Badge, DataTable, Modal, TextField, Divider,
...
export const action = async ({ request }) => {
  const { authenticate } = await import('../shopify.server.js');
  const { login, getAccounts, connectAccount, clearCredentials } = await import('../auth.server.js');
  ...
  if (intent === 'sign_in') {
    try {
      console.log('[CMPro] Attempting login for:', formData.get('username'));
      const token    = await login(formData.get('username'), formData.get('password'));
      console.log('[CMPro] Login succeeded, fetching accounts...');
      const accounts = await getAccounts(token);
      console.log('[CMPro] Accounts fetched:', accounts?.length);
          ...
          <BlockStack gap="400">
            ...
            {step === 'credentials' && (
              <BlockStack gap="300">
                ...
                <Button variant="primary" fullWidth onClick={handleSignIn} loading={loading || isSubmitting}>Connect</Button>
              </BlockStack>
            )}
            {step === 'accounts' && (
              <BlockStack gap="300">
